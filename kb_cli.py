...
def cmd_add(args: argparse.Namespace) -> int:
    from tangyuanAI.knowledge_base import (
...
def cmd_search(args: argparse.Namespace) -> int:
    from tangyuanAI.knowledge_base import search_sync
    results = search_sync(
...
def cmd_list(args: argparse.Namespace) -> int:
    from tangyuanAI.knowledge_base import list_kbs
    kbs = list_kbs()
    if not kbs:
        print("(无知识库)")
        return 0
    ...
    print("-" * 80)
    from tangyuanAI.knowledge_base import list_documents_sync
    for kb in kbs:
...
def cmd_show(args: argparse.Namespace) -> int:
    from tangyuanAI.knowledge_base import get_kb
    kb = get_kb(args.name)
...
def cmd_delete(args: argparse.Namespace) -> int:
    from tangyuanAI.knowledge_base import delete_kb
    if not args.yes:
...
def cmd_providers(args: argparse.Namespace) -> int:
    from tangyuanAI.knowledge_base import list_embedder_providers, list_reranker_providers
...
def cmd_processors(_args: argparse.Namespace) -> int:
    from tangyuanAI.knowledge_base import list_doc_processors
    print("Doc processors:", ", ".join(list_doc_processors()))
    return 0


def cmd_cache(args: argparse.Namespace) -> int:
    from tangyuanAI.knowledge_base import get_global_cache
